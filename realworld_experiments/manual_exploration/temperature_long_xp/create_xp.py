...
EXPERIMENT_DURATION_IN_SEC = 15*60
N_REPEATS = 6
RECIPY_INDEX_TO_RUN = [2, 3, 4, 6, 7, 8]
...
if __name__ == '__main__':
    ...
    for _ in range(N_REPEATS):
        for recipy_index in RECIPY_INDEX_TO_RUN:
            ## init XP_dict
            xp_folder = generate_next_XP_foldername(pool_folder)
            XP_dict = make_basic_XP_dict(xp_folder)

            ## insert oil recipy fields
            recipy = recipes[recipy_index]
            oil_ratios = {
                "dep": recipy[0],
                "octanol": recipy[1],
                "octanoic": recipy[2],
